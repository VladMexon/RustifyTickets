...
Windows-сборка добавляет portable-версию автоматически: в CI выгружается артефакт
`RustifyTickets-windows-portable` с двумя файлами — `RustifyTickets.exe` и
стандартным `categories.toml`. Распакуйте их в одну папку и запускайте — настройки
будут храниться рядом с exe, ничего в системе не создаётся.

При сборке по тегу оба этих файла прикладываются к релизу отдельным шагом
(`gh release upload`): `tauri-action` кладёт в релиз только установщики, поэтому
без него портативная версия оставалась лишь артефактом запуска.
...
2. **Сборка** — матрица: Linux (deb/rpm/AppImage), Windows (msi/exe), macOS (Apple Silicon и Intel).
   Запускается **только по тегу** или вручную, чтобы обычные коммиты не расходовали
   минуты раннеров

Запускается при:

- пуше в `master`/`main` и в pull request — **только тесты ядра**, сборка не запускается
- пуше тега вида `v1.2.3` — тесты, затем полная сборка на трёх ОС и **черновик релиза**
  со всеми установщиками
- вручную через **Actions → Build → Run workflow** с включённым флагом `build` —
  сборка без релиза, если нужно проверить сборку, не выпуская версию

Артефакты запуска (появляются у теговых и ручных сборок):
...
Артефакты хранятся 30 дней и скачиваются только после входа в GitHub.
...
Номер тега должен совпадать с версией приложения, иначе файлы в релизе будут
называться по версии из конфига. Повторно использовать номер уже выпущенного тега
не стоит: ассеты с теми же именами конфликтуют при загрузке — выпускайте следующим
номером.
